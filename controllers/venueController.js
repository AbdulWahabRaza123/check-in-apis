...
exports.createVenue = async (req, res) => {
  try {
    const venue = await venueService.createVenue(req.body, req.files.files, JSON.parse(req.body.venueCities));
    res.status(201).json(venue);
  } catch (error) {
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
...
exports.getVenuesWithinRadiusAndCity = async (req, res) => {
  try {
    const { latitude, longitude, radius, city } = req.query;
    const venues = await venueService.getVenuesWithinRadiusAndCity(latitude, longitude, radius, city);
    res.status(200).json(venues);
  } catch (error) {
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
...
exports.getVenueById = async (req, res) => {
  try {
    const { venueId } = req.params;
    const currentTime = req.query.currentTime; // Assuming the currentTime is passed as query param
    const venueDetails = await venueService.getVenueById(venueId, currentTime);
    res.status(200).json(venueDetails);
  } catch (error) {
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
